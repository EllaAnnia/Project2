...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label3->Text = L"Розв\'язання";
			// 
			// label4
			// 
...
private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	// Перевірка різних інтервалів на наявність кореня
	for (double i = a; i < b; i += 0.5) {
		if (Function(i) * Function(i + 0.5) < 0) {
			// Якщо знайшли інтервал із різними знаками на кінцях, використовуємо його
			a = i;
			b = i + 0.5;
			rootFound = true;
			break;
	...
	if (!rootFound) {
		textBox4->Text = "Корінь не знайдено на інтервалі [-10, 10]";
		return;
	}
